...
class PsnPriceSpider(scrapy.Spider):
    ...
    start_urls = [
        ...
        'https://store.playstation.com/en-ca/grid/STORE-MSF77008-PS3PSNPREORDERS/1',
    ...
    def parse(self, response):
        ...
        for grid in response.css(GRID_SELECTOR):
            ...
            item_title_name = grid.css(TITLE_NAME_SELECTOR).extract_first()
            item_title_url: 'https://store.playstation.com' + \
                grid.css(URL_SELECTOR).extract_first()
            item_thumb_img_url: grid.css(THUBM_IMG_SELECTOR).extract_first()
            ...
            titleItem.append({'id': grid.css(URL_SELECTOR).extract_first().replace("/en-ca/product/", ""),
                              ...
                              'thumb_img_url': grid.css(THUBM_IMG_SELECTOR).extract_first().split(", ")[2],
